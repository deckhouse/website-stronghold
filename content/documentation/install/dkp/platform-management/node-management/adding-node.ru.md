...
Пример добавления статического узла в кластер с помощью [Cluster API Provider Static (CAPS)](../node-group/#настройка-узла-через-caps):
...
{{< alert level="warning" >}}
Если на зачищаемом узле есть пулы хранения LINSTOR/DRBD, то следуйте [инструкции](/modules/sds-replicated-volume/stable/faq.html#как-вытеснить-drbd-ресурсы-с-узла) модуля `sds-replicated-volume`, чтобы выгнать ресурсы с узла и удалить узел LINSTOR/DRBD.
{{< /alert >}}
...
Если узел находится под управлением [CAPS](../node-group/#настройка-узла-через-caps), то изменить принадлежность к NodeGroup у такого узла **нельзя**. Единственный вариант — [удалить StaticInstance](#можно-ли-удалить-staticinstance) и создать новый.
