...
Для применения новых параметров после изменения файла конфигурации перезапустите сервис Stronghold:
...
* `cache_size` — необязательный параметр (строка). Размер кеша чтения, используемого физической подсистемой хранения.
  Указывается в числе записей, поэтому итоговый объём кеша зависит от их размера.
  Значение по умолчанию — `131072`.

* `disable_cache` — необязательный логический параметр. Полностью отключает кеширование в Stronghold, включая кеш чтения.
...
* `tls_13_cipher_policy` — необязательный строковый параметр. Управляет порядком предпочтения наборов шифров TLS 1.3 на уровне процесса, позволяя использовать ГОСТ-шифры (Магма / Кузнечик) вместе с AES/ChaCha20 или вместо них. В отличие от `tls_cipher_suites` в секции `listener`, который влияет только на TLSv1.2 и ниже, этот параметр действует на TLS 1.3. Возможные значения:
  * `auto` (по умолчанию) — если хотя бы один TLS 1.3-слушатель использует ГОСТ-сертификат, работает как `prefer-gost`, иначе — как `prefer-aes`.
  * `prefer-gost` — сначала ГОСТ-шифры, затем AES/ChaCha20.
  * `prefer-aes` — сначала AES/ChaCha20, затем ГОСТ-шифры.
  * `gost-only` — разрешены только ГОСТ-шифры.
  * `aes-only` — разрешены только AES/ChaCha20; ГОСТ отключён.

  Порядок наборов шифров TLS 1.3 в Go является глобальным для процесса, поэтому настройка применяется ко всему процессу Stronghold (всем его слушателям и исходящим соединениям TLS 1.3), а не к отдельному слушателю.
  На CLI, agent и proxy, работающие как отдельные процессы, она не влияет — у каждого из них свой параметр `tls_13_cipher_policy`.
  При разрешении `auto` и проверке строгих режимов учитываются только слушатели с включённым TLS и `tls_max_version`, равным `"tls13"` (или не заданным).
  Настройка пересчитывается при перезагрузке по `SIGHUP`.

  ГОСТ-шифры записей (Magma / Kuznechik) доступны **только в TLS 1.3**. Не задавайте `tls_max_version = "tls12"` на слушателях с ГОСТ-сертификатами — это отключит ГОСТ TLS на этих слушателях.
...
## Секция user_lockout {#user_lockout}
