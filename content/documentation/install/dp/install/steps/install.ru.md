...
### Файл конфигурации установки {#config-file}
...
{{% details summary="Пример файла конфигурации установки (config.yaml)..." %}}
...
**Внимание!** В файле ресурсов установки нельзя использовать ModuleConfig для **встроенных** модулей. Используйте для них [файл конфигурации](#config-file).

{{% details summary="Пример файла ресурсов (resources.yaml)..." %}}
...
> При установке платформы, отличной от [редакции Community Edition](../../../../about/editions/), из официального container registry `registry.deckhouse.ru` необходимо предварительно авторизоваться с помощью лицензионного ключа:
>
> ```shell
...
- `<REVISION>` — [редакция](../../../../about/editions/) платформы (например `ee` — для Enterprise Edition, `ce` — для Community Edition и т. д.)
- `<MOUNT_OPTIONS>` — параметры монтирования файлов в контейнер инсталлятора, таких как:
  - SSH-ключи доступа;
  - файл конфигурации;
  - файл ресурсов и т. д.
- `<RELEASE_CHANNEL>` — [канал обновлений](../../release-channels/) платформы в kebab-case. Должен совпадать с установленным в `config.yaml`:
