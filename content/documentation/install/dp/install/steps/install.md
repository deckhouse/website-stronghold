...
> When installing a platform edition other than the [Community Edition](../../../../about/editions/) from the official container registry `registry.deckhouse.io`, you need to authenticate with a license key beforehand:
>
> ```shell
...
- `<MOUNT_OPTIONS>` — options for mounting files into the installer container, such as:
  ...
  - configuration file;
  - resources file, etc.
- `<RELEASE_CHANNEL>` — [release channel](../../release-channels/) of the platform in kebab-case. It should match the one set in `config.yaml`:
