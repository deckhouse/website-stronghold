...
Подробно о составе аудит-записи и формате хранения данных см. на странице [Схема записей журнала аудита](./log-format/).
...
- [Схема записей журнала аудита](./log-format/) — структура audit-записи, ключевые объекты `auth`, `request`, `response` и защита чувствительных данных;
- [Фильтрация данных аудита](./audit-filtering/) — позволяет направлять в конкретное устройство только записи, соответствующие условию;
- [Исключение полей из данных аудита](./audit-exclusion/) — позволяет удалять отдельные поля из записей перед их сохранением.
