...
|----------------|----------------------|--------|
| **Harden** | **Высокая** | Криптография (TLS, barrier, seal wrap), аутентификация (в т.ч. [MFA](../../user/auth/mfa), [WebAuthn](../../user/auth/web-auth-n) / FIDO2 и passkeys, безпарольный вход в UI), динамические секреты, Managed Keys (`pkcs11`, `yandexcloudkms`). |
| **Isolate** | **Средняя–высокая** | ACL, default deny, [namespaces](../basic-exploitation/namespaces), квоты с **rate limit** на API (ограничение частоты запросов по пути/монтированию/пространству имён). |
...
| **Restore** | **Средняя–высокая** | Повторная выдача динамических учётных данных и сертификатов (**Reissue Credential**); восстановление кластера из **снимков Raft** ([автоснимки](../reliability-and-recovery/automated-snapshots), ручной [снимок](../reliability-and-recovery/snapshots)); настройка политики бэкапов через **API с `sudo`**. |
...
**Многофакторная аутентификация** усиливает защиту входа в Stronghold и косвенно снижает сценарии с одним украденным паролем; подробности — в разделе 2FA / MFA ([TOTP](../../user/auth/mfa/totp), [Multifactor LDAP](../../user/auth/mfa/mfla)). Отдельно [метод WebAuthn](../../user/auth/web-auth-n) опирается на **FIDO2** и **passkeys** и подходит для **безпарольного** входа в UI и интеграций по HTTP API (эндпоинты `login`/`register` описаны там же). В таблице это отражено прежде всего в строках **T1110** и **T1078**.
...
| [TA0040](https://attack.mitre.org/tactics/TA0040) Impact | [T1485](https://attack.mitre.org/techniques/T1485) Data Destruction | Строгие политики к путям с облачными/инфраструктурными ключами и секретами снижают риск несанкционированного получения полномочий на разрушительные действия. После инцидента восстановление данных Stronghold из **снимков Raft** (см. [восстановление](../reliability-and-recovery/recovery)) уменьшает необратимые последствия для кластера при корректных внешних копиях и процедурах. |
