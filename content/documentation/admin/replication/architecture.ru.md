...
| Sealwrapper | отдельный блок сбоку | seal — механизм, который хранит ключ для слоя seal wrap; свой у каждого кластера |
...
| Sealwrap | малиновый | только EE | второй слой шифрования для чувствительных путей; ключ хранит seal |
...
| Disaster Recovery | Между кластерами | Горячий резерв и аварийное переключение | secondary не обслуживает клиентов, ждёт promote | EE |
| Performance | Между кластерами | Масштабирование и распределение чтений | secondary читают локально, запись — на primary | EE |
...
- **Sealwrap** стоит в самом низу, между Raft и физическим хранилищем. Это второй слой шифрования для чувствительных путей; ключ хранит seal (блок `Sealwrapper` сбоку).
...
Межкластерная репликация доступна только в EE. Она связывает целые кластеры: primary отдаёт поток журнала на secondary (синие Data WAL Streaming), а secondary перенаправляет клиентские записи обратно на primary. У каждого кластера свой `Sealwrapper` — и он может отличаться.
