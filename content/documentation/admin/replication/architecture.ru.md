...
Схемы упрощены: они показывают состав узла и направление запросов, а не сетевую разводку. Термины «узел» и «кластер», а также сценарии репликации разобраны на странице [Топологии и сценарии](../topologies/).
...
- **Работает на уровне ноды.** Так как общение идёт через API-ручки конкретного узла, репликация настраивается на конкретную ноду кластера, а не на кластер целиком.
...
Ниже — несколько примеров таких комбинаций. Они показывают, что размер (HA или одна нода) выбирается для каждого кластера отдельно, а KV-репликацию можно навесить на любой кластер, кроме DR secondary.

![Комбинированная топология: одно-нодовый primary, HA-secondary и KV-репликация](/images/stronghold/stronghold-topology-combined-1.png "Primary DR+Performance из одной ноды, HA-secondary, KV-репликация в primary")

Primary одновременно DR- и Performance-primary и состоит из одной ноды; оба secondary — HA-кластеры. Отдельный кластер подаёт данные в primary по KV-репликации.
