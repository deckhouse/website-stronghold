...
- Поля `allowed_parameters`, `denied_parameters` и `required_parameters` не применимы к полям секрета в механизме секретов KV/v2: KV/v2 передаёт их вложенными в `data`, поэтому правило, написанное на имя поля секрета, ни с чем не совпадёт. К параметрам строки запроса это не относится.
{{< /alert >}}
...
Рекурсивный list возвращает все ключи по указанному пути, включая ключи во вложенных каталогах, одним плоским списком.

Обход доходит ровно туда же, куда дошёл бы обход каталог за каталогом.

Во вложенный каталог обход спускается только там, где у токена есть право выполнять операцию list.

Поддерево, защищённое отсутствием права list, остаётся закрытым.

Рекурсия — не отдельное право. Её запрашивают параметром `recursive`, и на него распространяются описанные выше ограничения параметров.

Отличие от остальных параметров одно: рекурсия выдаётся явно. Политика, которая о параметре молчит, разрешает только обычную операцию list.
...
Без этого правила путь по-прежнему листается по одному каталогу за раз, а рекурсивный запрос отклоняется с кодом `403`.

Секция, допускающая любые параметры через `allowed_parameters = { "*" = [] }`, выдаёт и рекурсию.

Разрешение действует на всё поддерево, которому соответствует правило, поэтому для вложенных каталогов повторять его не нужно. Чтобы закрыть один из них от рекурсии, достаточно более специфичного правила, которое о параметре не говорит ничего: путь оценивается по одному, самому специфичному подходящему правилу, а оно рекурсию не выдаёт.
...
В этом примере рекурсивный list по пути `kv/` вернёт поддерево, кроме `kv/private`: в этот каталог обход не заходит. Обычная операция list по пути `kv/private/` при этом работает.

Запрет `denied_parameters = { "recursive" = [] }` нужен там, где выигравшее правило рекурсию всё-таки выдаёт: он сужает разрешение по маске и перекрывает другую политику, выдающую параметр на том же пути.
...
Правило действует везде, куда доходит обход, а не только в точке старта. Если пути соответствует несколько политик, достаточно разрешения в любой из них, а запрет в любой из них перевешивает.

Рекурсию запрашивают самим наличием параметра, а не его значением, поэтому пустой список — единственная допустимая форма. Правило со значениями отклоняется при записи политики, так как оно никогда не совпало бы с запросом.
...
При обходе проверяется путь запроса вместе с вложенным каталогом, операция всегда list. Право `read` не запрашивается: обычная операция list показывает имена секретов, которые токен не может прочитать, и рекурсивная держится того же правила. Не используйте конфиденциальную информацию в именах ключей.

Обычная операция list не несёт параметров, поэтому ограничения параметров её не касаются.

Параметр `recursive` передаётся в строке запроса, поэтому на путях KV/v2 он учитывается: оговорка выше о полях секрета его не затрагивает.

{{< alert level="info" >}}
Рекурсивный list должен поддерживаться самим путём. Он поддерживается механизмами секретов KV/v1 и KV/v2, на остальных путях запрос отклоняется с кодом `400`. Механизм секретов, работающий как внешний плагин, рекурсивный list обслуживать не может — такой запрос также отклоняется с кодом `400`.
{{< /alert >}}

## Требуемые TTL для обёртывания ответа

Эти параметры можно использовать для установки минимальных и максимальных значений TTL, задаваемых клиентами при запросе на [обёртывание ответа](response-wrapping/), с точностью до секунды. Они используют строки формата `duration`.
