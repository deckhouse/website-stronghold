...
Метод `ldap` auth позволяет выполнять аутентификацию с использованием существующего сервера LDAP и учетных данных - имени пользователя и пароля. Это позволяет интегрировать Stronghold в среды, использующие LDAP. Stronghold поддерживает интеграцию с различными службами каталогов LDAP, в том числе и с российской службой каталогов [ALD Pro](https://www.aldpro.ru/).
