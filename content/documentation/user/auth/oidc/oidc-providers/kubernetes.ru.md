...
Kubernetes может выступать в качестве OIDC-провайдера, чтобы Stronghold мог проверять токены `ServiceAccount` с помощью `JWT/OIDC auth`.
...
- вы хотите использовать `JWT auth`, а не `Kubernetes auth`.
...
Это наиболее простой вариант, если ваш кластер Kubernetes поддерживает автоматическую публикацию OIDC metadata.
...
4. Настройте роли, как описано ниже в разделе **Создание ролей и аутентификация**.
...
- вам удобнее использовать цепочку публичных ключей напрямую.
...
4. Настройте роли, как описано ниже.
...
Если требуется задать собственный TTL или audience для токенов `ServiceAccount`, можно использовать отдельное монтирование `serviceAccountToken` в поде.
