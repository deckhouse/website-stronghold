...
Этот метод подходит, если клиент уже может получить JWT от доверенного issuer и передать его в Stronghold без запуска браузерного OIDC-сценария.

> Примечание  
> Если вам нужен интерактивный вход через браузер и OIDC-провайдера, используйте раздел [OIDC](./oidc/overview/). В `JWT`-сценарии Stronghold не инициирует browser-based flow, а только валидирует уже выданный токен.
...
- при успешной проверке выдаёт токен Stronghold.

В результате пользователь или приложение получают обычный токен Stronghold, с которым можно выполнять дальнейшие операции.

## Проверка JWT

Подписи JWT проверяются по открытым ключам issuer. Для одного backend'а можно выбрать один из следующих способов проверки:
...
- **OIDC Discovery** — используется URL OIDC Discovery, из которого извлекаются ключи и применяются критерии проверки OIDC, например `iss` и `aud`.

Если требуется использовать несколько способов проверки, необходимо поднять несколько экземпляров backend'а `JWT auth`.
...
Если backend JWT смонтирован по другому пути, используйте его вместо `jwt`.

## Аутентификация через API

По умолчанию используется endpoint `auth/jwt/login`. Если метод аутентификации включён по другому пути, замените `jwt` на нужное значение.
...
В ответе Stronghold вернёт токен по адресу `auth.client_token`.
...
Перед тем как аутентифицироваться, необходимо включить и настроить backend `JWT auth`. Это обычно делает администратор или средство управления конфигурацией.
...
Backend будет смонтирован по выбранному имени.
...
- локальных ключей;
- либо URL `JWKS`;
- либо URL `OIDC Discovery`.

Для ролей `OIDC` дополнительно требуются `oidc_client_id` и `oidc_client_secret`, но для сценария `JWT` они могут оставаться пустыми.
...
Если Stronghold должен только валидировать JWT, оставьте `oidc_client_id` и `oidc_client_secret` пустыми:
...
- использует указанные claims пользователя и групп для настройки identity-псевдонимов.

## Связанные параметры

После того как JWT подтверждён как корректно подписанный и неистёкший, Stronghold проверяет все настроенные связанные параметры роли.
...
Данные claims можно копировать в метаданные токена аутентификации и alias с помощью `claim_mappings`.
...
- значение claim `department` копируется в одноимённый ключ `department`.
...
могут ссылаться на данные внутри JWT.

Если нужный claim расположен на верхнем уровне JWT, можно указать его имя напрямую. Если он вложен глубже, можно использовать [JSON Pointer](https://tools.ietf.org/html/rfc6901).
