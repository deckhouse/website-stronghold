...
1. `env_template` вместе с `exec` — рендеринг в переменные окружения и запуск процесса. Agent формирует значения переменных окружения и запускает приложение как дочерний процесс. Если секреты меняются, процесс можно перезапустить.
...
1. Подставляет реальные значения в шаблон.
1. Сохраняет итоговый файл с нужными правами доступа.
1. При необходимости выполняет команду для перезагрузки приложения.
...
- приложения без поддержки API Stronghold;
- доставка секретов в `.properties`, `.conf`, `.ini`, `.yaml`;
- работа с динамическими credentials для баз данных;
...
Сценарий: legacy Java-приложение читает credentials базы данных из `application.properties`.
...
1. получает токен и сохраняет его в sink;
1. запрашивает секреты;
1. рендерит шаблон и создаёт `/etc/myapp/application.properties`;
...
#### Динамические credentials для баз данных
...
- получает credentials базы данных и API-ключ;
...
{% alert level="info" %}
Блок `env_template` всегда имеет имя переменной окружения: `env_template "MY_VAR" { ... }`. Поля `destination`, `perms`, `command`, `wait` и похожие параметры в `env_template` не поддерживаются [3].
{% endalert %}

### Управление жизненным циклом процесса

Когда секреты меняются, например при ротации credentials базы данных, Agent:

1. получает новые секреты;
1. формирует новые переменные окружения;
1. отправляет `SIGTERM` дочернему процессу;
1. перезапускает процесс с обновлёнными значениями.
...
Stronghold Agent может работать как прокси для API Stronghold.
...
В этом сценарии приложение обращается к локальному Agent, а Agent сам добавляет токен и проксирует запрос на сервер Stronghold.
...
1. Сам проходит аутентификацию в Stronghold.
1. Получает токен и использует его для templating, API Proxy и других задач.
1. Если настроен sink, записывает токен в файл.
1. Продлевает токен до истечения TTL.
1. При необходимости проходит аутентификацию заново.

### Sink

Sink — это место, куда Agent записывает полученный токен.

Если sink настроен, токен записывается в файл, например `/var/run/stronghold-agent/token`, и его могут использовать другие процессы. Если sink не настроен, токен используется только самим Agent.
...
У AppRole есть следующие преимущества:
...
### Настройка на стороне Stronghold
...
- обычно он хранится в `/etc/stronghold-agent/role-id`;
- после использования он не удаляется;
...
- обычно он хранится в `/etc/stronghold-agent/secret-id`;
- при необходимости может быть удалён после чтения;
- его не следует хранить в Git или в системе управления конфигурацией без дополнительной защиты.
...
- это наиболее безопасный вариант для production-окружения.
...
У этого метода есть следующие ограничения:
...
JWT/OIDC-аутентификация позволяет использовать существующую систему identity management для входа в Stronghold.

### Как это работает

1. Приложение получает JWT от identity provider.
1. JWT содержит claims о пользователе или сервисе.
1. Stronghold проверяет подпись и извлекает claims.
1. На основе claims выдаёт собственный токен Stronghold.
...
- использования service account от identity provider;
- федеративной аутентификации;
- CI/CD через OIDC, например GitHub Actions или GitLab CI.

### Преимущества

JWT/OIDC даёт следующие преимущества:
...
Настройка метода на стороне Stronghold:
...
Если JWT истекает, нужно получить новый токен от identity provider. В исходных материалах для этого предлагается отдельное периодическое обновление.
