...
Чтобы получить все поддерево одним запросом, необходимо добавить флаг `-recursive`:
...
Результат операции включает в себя только те каталоги, которые разрешены для обычной операции list.

Поддерево, недостижимое при пошаговом листинге, в ответ не попадёт.
