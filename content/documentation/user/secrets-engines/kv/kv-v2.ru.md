...
Поля `allowed_parameters`, `denied_parameters` и `required_parameters` не применимы к полям секрета в хранилище kv версии 2. KV/v2 передаёт их вложенными в `data`, поэтому правило, написанное на имя поля секрета, ни с чем не совпадёт.

К параметрам строки запроса это не относится. Например, параметр `recursive` на путях `metadata/` разрешает операцию рекурсивный list.

Описание этих параметров приведено в разделе [Политики](../../../concepts/policy/).
...
По умолчанию операция list возвращает только непосредственные дочерние ключи.

Чтобы получить всё поддерево одним запросом, необходимо добавить флаг `-recursive`:
...
Ключи возвращаются как пути относительно запрошенного.

В хранилище KV версии 2 обход идёт по дереву метаданных, поэтому параметры политики задаются для пути `metadata/`, а не `data/`.

Для предоставления возможности выполнения операции рекурсивный list необходимо добавить параметр `allowed_parameters = { "recursive" = [] }`.
...
Результат операции включает в себя только те каталоги, которые разрешены для обычной операции list на путях метаданных.

Правило, написанное для пути `data/`, рекурсию не выдаёт и не ограничивает.

Запрет `denied_parameters = { "recursive" = [] }` отбирает рекурсию на пути, оставляя обычный листинг работать. Подробнее — в разделе [«Рекурсивный list»](../../../concepts/policy/#рекурсивный-list).

За один запрос возвращается не больше 10 000 ключей и просматривается не больше 100 000 записей хранилища.

При достижении любого из ограничений Stronghold возвращает собранные ключи и добавляет предупреждение о неполноте результата.
