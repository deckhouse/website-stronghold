...
- Рекурсивный list для механизмов секретов KV/v1 и KV/v2: флаг `-recursive` возвращает всё поддерево пути одним запросом. Операция разрешается параметром политики `allowed_parameters = { "recursive" = [] }` в дополнение к праву `list`, а обход доходит только до каталогов, которые разрешено листать токену.
