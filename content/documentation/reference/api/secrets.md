...
### POST /{gitops_mount_path}/configure/git_credential

**Operation ID:** `gitops-write-configure-git_credential`
...
| Parameter | Type | Location | Required | Description |
|----------|-----|--------------|--------------|----------|
| `gitops_mount_path` | string | path | yes | Path that the backend was mounted at |
...
### DELETE /{gitops_mount_path}/configure/git_credential

**Operation ID:** `gitops-delete-configure-git_credential`
...
### GET /{gitops_mount_path}/configure/git_repository

**Operation ID:** `gitops-read-configure-git_repository`

Read the current gitops git_repository configuration.
...
### POST /{gitops_mount_path}/configure/git_repository

**Operation ID:** `gitops-write-configure-git_repository`

Update the current gitops git_repository configuration.
...
| Parameter | Type | Required | Description |
|----------|-----|--------------|----------|
| `git_branch_name` | string (default: main) | no | Git repo branch |
| `git_ca_certificate` | string | no | Git CA. Default is empty. |
| `git_poll_period` | integer (default: 5m) | no | Period between polls of Git repo |
| `git_repo_url` | string | no | Git repo URL. Required for CREATE. |
| `max_clone_size_bytes` | integer (default: 10485760) | no | Max size (bytes) of in-memory clone; clone fails when exceeded (0 = no limit). Protects against OOM on large or malicious repos. |
| `required_number_of_verified_signatures_on_commit` | integer (default: 0) | no | Verify that the commit has enough verified signatures |

#### Responses

**200**: OK

### DELETE /{gitops_mount_path}/configure/git_repository

**Operation ID:** `gitops-delete-configure-git_repository`

Delete the current gitops git_repository configuration.
...
### GET /{gitops_mount_path}/configure/gitops

**Operation ID:** `gitops-read-configure-gitops`

Read gitops configuration.
...
### POST /{gitops_mount_path}/configure/gitops

**Operation ID:** `gitops-write-configure-gitops`
...
### DELETE /{gitops_mount_path}/configure/gitops

**Operation ID:** `gitops-delete-configure-gitops`
...
### GET /{gitops_mount_path}/configure/trusted_pgp_public_key

**Operation ID:** `gitops-list-configure-trusted_pgp_public_key`

List trusted PGP public keys
...
### GET /{gitops_mount_path}/configure/trusted_pgp_public_key/{name}

**Operation ID:** `gitops-read-configure-trusted_pgp_public_key-name`

CRUD operations for trusted PGP public key
...
### POST /{gitops_mount_path}/configure/trusted_pgp_public_key/{name}

**Operation ID:** `gitops-write-configure-trusted_pgp_public_key-name`
...
| Parameter | Type | Required | Description |
|----------|-----|--------------|----------|
| `public_key` | string | no | Key data (required for CREATE/UPDATE) |

#### Responses

**200**: OK

### DELETE /{gitops_mount_path}/configure/trusted_pgp_public_key/{name}

**Operation ID:** `gitops-delete-configure-trusted_pgp_public_key-name`
...
### GET /{gitops_mount_path}/status

**Operation ID:** `gitops-read-status`
