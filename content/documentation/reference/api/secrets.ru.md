...
**ID операции:** `gitops-write-configure-git_credential`

Настроить учетные данные Git
...
| Параметр | Тип | Расположение | Обязательный | Описание |
|----------|-----|--------------|--------------|----------|
| `gitops_mount_path` | string | path | да | Путь, по которому был смонтирован бэкэнд |

#### Параметры тела запроса

| Параметр | Тип | Обязательный | Описание |
|----------|-----|--------------|----------|
| `password` | string | нет | Пароль Git; обязателен для CREATE, UPDATE. |
| `username` | string | нет | Имя пользователя Git; обязательно для CREATE, UPDATE. |
...
**ID операции:** `gitops-delete-configure-git_credential`
...
**ID операции:** `gitops-read-configure-git_repository`

Прочитать текущую конфигурацию git_repository для gitops.
...
**ID операции:** `gitops-write-configure-git_repository`

Обновить текущую конфигурацию git_repository для gitops.
...
| Параметр | Тип | Обязательный | Описание |
|----------|-----|--------------|----------|
| `git_branch_name` | string (default: main) | нет | Ветка Git-репозитория |
| `git_ca_certificate` | string | нет | CA Git. По умолчанию пусто. |
| `git_poll_period` | integer (default: 5m) | нет | Интервал между опросами Git-репозитория |
| `git_repo_url` | string | нет | URL Git-репозитория. Обязателен для CREATE. |
| `max_clone_size_bytes` | integer (default: 10485760) | нет | Максимальный размер (в байтах) клона в памяти; при превышении клонирование завершается ошибкой (0 = без ограничения). Защита от OOM на больших или вредоносных репозиториях. |
| `required_number_of_verified_signatures_on_commit` | integer (default: 0) | нет | Проверять, что у коммита достаточно верифицированных подписей |
...
**ID операции:** `gitops-delete-configure-git_repository`

Удалить текущую конфигурацию git_repository для gitops.
...
**ID операции:** `gitops-read-configure-gitops`

Прочитать конфигурацию gitops.
...
**ID операции:** `gitops-write-configure-gitops`

Обновить конфигурацию gitops.
...
| Параметр | Тип | Обязательный | Описание |
|----------|-----|--------------|----------|
| `path` | string (default: ) | нет | Путь к YAML-файлам в репозитории (например, `vault` или пусто для корня). |
...
**ID операции:** `gitops-delete-configure-gitops`

Удалить конфигурацию gitops.
...
**ID операции:** `gitops-list-configure-trusted_pgp_public_key`

Получить список доверенных открытых ключей PGP
...
**ID операции:** `gitops-read-configure-trusted_pgp_public_key-name`

CRUD-операции для доверенного открытого ключа PGP
...
**ID операции:** `gitops-write-configure-trusted_pgp_public_key-name`
...
| Параметр | Тип | Обязательный | Описание |
|----------|-----|--------------|----------|
| `public_key` | string | нет | Данные ключа (обязательны для CREATE/UPDATE) |
...
**ID операции:** `gitops-delete-configure-trusted_pgp_public_key-name`
...
**ID операции:** `gitops-read-configure-vault`

Прочитать текущую конфигурацию клиента Vault.
...
**ID операции:** `gitops-write-configure-vault`

Обновить текущую конфигурацию клиента Vault.
...
| Параметр | Тип | Обязательный | Описание |
|----------|-----|--------------|----------|
| `rotate` | boolean (default: False) | нет | Ротировать токен Vault. |
| `vault_addr` | string | нет | Адрес API Vault. |
| `vault_cacert_bytes` | string | нет | CA-сертификат в кодировке PEM для проверки TLS Vault. |
| `vault_namespace` | string | нет | Пространство имен Vault для доступа к API. |
| `vault_token` | string | нет | Токен Vault для доступа к API. |
| `wrapping_token` | string | нет | Оборачивающий токен Vault для доступа к API. |
...
**ID операции:** `gitops-delete-configure-vault`

Удалить текущую конфигурацию клиента Vault.
...
**ID операции:** `gitops-read-status`
...
| `list` | string | query | нет | Возвращает список, если `true`. |
| `recursive` | boolean | query | нет | Возвращать все ключи под путём, а не только непосредственных потомков как пути относительно него. Требует `list=true` и возможность `allow_recursive` для пути. |
...
| `challenge_excluded_ip_ranges` | array (default: []) | нет | Список блоков CIDR, исключённых из проверки вызова ACME. IP-адреса в этих диапазонах будут отклонены при проверке. Могут быть отдельные IP или запись CIDR. Этот список имеет приоритет над challenge_permitted_ip_ranges. |
| `challenge_permitted_ip_ranges` | array (default: []) | нет | Список блоков CIDR, разрешённых для проверки вызова ACME. Если установлено, только IP-адреса в этих диапазонах будут допущены к проверке. Могут быть отдельные IP или запись CIDR. |
...
| `other_sans` | array | нет | Запрошенные дополнительные SAN в массиве в формате <oid>;<type>:<value> для каждой записи, где <type> является одним из UTF8, IA5 и KRB5. Значение KRB5 — это principal Kerberos, например host/kdc.example.com@EXAMPLE.COM, и определено только для OID 1.3.6.1.5.2.2. |
...
| `allowed_other_sans` | array | нет | Если установлено, массив разрешённых альтернативных имён для размещения в SAN. Эти значения поддерживают подстановочные символы и должны быть в формате <oid>;<type>:<value>. Допустимые типы: "utf8" для otherName с UTF8String, "ia5" для otherName с IA5String, и "krb5" для KRB5PrincipalName RFC 4556, которая определена только для OID 1.3.6.1.5.2.2, и значение записывается в обычном формате Kerberos, как в "1.3.6.1.5.2.2;krb5:host/*@EXAMPLE.COM". Значение сопоставляется только с записями его собственного типа. Все значения, включая подстановочные, должны использовать этот синтаксис, за исключением одного символа "*", который допускает любой OID, тип и значение. |
...
| Параметр | Тип | Обязательный | Описание |
|----------|-----|--------------|----------|
| `generate_signing_key` | boolean (default: True) | нет | Сгенерировать пару SSH-ключей внутри системы вместо использования полей private_key и public_key. Если указана конфигурация управляемого ключа, это поле игнорируется. |
...
**ID операции:** `trdl-read-configure`
...
| Параметр | Тип | Расположение | Обязательный | Описание |
|----------|-----|--------------|--------------|----------|
| `trdl_mount_path` | string | path | да | Путь, по которому был смонтирован бэкэнд |
...
**ID операции:** `trdl-write-configure`
...
| Параметр | Тип | Обязательный | Описание |
|----------|-----|--------------|----------|
| `buildkitd_address` | string | нет | Адрес запущенного buildkitd (scheme unix://, tcp://, docker-container:// или kube-pod://) для сборки артефактов релиза с помощью клиента BuildKit; docker CLI используется только если ни это, ни buildkitd_driver не установлены. Секреты сборки отправляются этому демону, а tcp:// не зашифрован и не аутентифицирован, поэтому ответственность за защиту канала и изоляцию демона лежит на администраторе. |
| `buildkitd_driver` | string | нет | Подготовить эфемерный buildkitd для каждой сборки вместо использования docker CLI: kubernetes запускает его как pod и не требует docker binary рядом с плагином. Не может быть скомбинирован с buildkitd_address, buildx_driver или buildx_driver_opts. TRDL_BUILDKITD_ADDRESS, установленный в процессе, имеет приоритет над сохранённым драйвером, и сборка указывает драйвер как неиспользуемый. |
| `buildkitd_driver_opts` | array | нет | Опции драйвера buildkitd, по одной паре name=value на элемент (например, namespace=trdl-build); требуют установки buildkitd_driver. Драйвер kubernetes принимает annotations, deadline, image, labels, limits.cpu, limits.ephemeral-storage, limits.memory, namespace, nodeselector, requests.cpu, requests.ephemeral-storage, requests.memory, rootless, serviceaccount и timeout; остальное отклоняется. Если deadline не установлен, по умолчанию используется оставшееся время задачи релиза при создании pod плюс пятиминутный запас, так что крах плагина не может оставить pod builder работающим бесконечно. |
| `buildx_driver` | string | нет | Драйвер buildx для сборки артефактов релиза: docker-container (используется по умолчанию) или kubernetes. Имеет приоритет над переменной окружения TRDL_BUILDX_DRIVER и не может быть скомбинирован с buildkitd_address или buildkitd_driver. |
| `buildx_driver_opts` | array | нет | Опции драйвера buildx, по одному --driver-opt на элемент (например, namespace=trdl-build), передаются как есть. Имеют приоритет над переменными окружения TRDL_BUILDX_DRIVER_OPTS_*, и не могут быть скомбинированы с buildkitd_address или buildkitd_driver. |
| `git_repo_url` | string | да | URL Git-репозитория |
| `git_trdl_channels_branch` | string | нет | Специальная ветка Git для хранения файла конфигурации каналов trdl |
| `git_trdl_channels_path` | string | нет | Путь в Git-репозитории к файлу конфигурации каналов trdl (по умолчанию используется trdl_channels.yaml) |
| `git_trdl_path` | string | нет | Путь в Git-репозитории к файлу конфигурации релиза trdl (по умолчанию используется trdl.yaml) |
| `initial_last_published_git_commit` | string | нет | Начальный коммит последней успешной публикации |
| `required_number_of_verified_signatures_on_commit` | integer | да | Требуемое число верифицированных подписей для коммита |
| `s3_access_key_id` | string | да | Идентификатор ключа доступа хранилища S3 |
| `s3_bucket_name` | string | да | Имя контейнера хранилища S3 |
| `s3_endpoint` | string | да | Конечная точка хранилища S3 |
| `s3_region` | string | да | Регион хранилища S3 |
| `s3_secret_access_key` | string | да | Секретный ключ доступа хранилища S3 |
...
**ID операции:** `trdl-delete-configure`
...
**ID операции:** `trdl-write-configure-build-mac_signing_identity`

Добавить или обновить учетные данные для подписи сборки
...
| Параметр | Тип | Обязательный | Описание |
|----------|-----|--------------|----------|
| `data` | string | да | Данные сертификата в кодировке base64 |
| `notary_issuer` | string | да | Издатель Notary |
| `notary_key` | string | да | Ключ Notary в кодировке base64 |
| `notary_key_id` | string | да | Идентификатор ключа Notary |
| `password` | string | да | Пароль сертификата |
...
**ID операции:** `trdl-delete-configure-build-mac_signing_identity`
...
**ID операции:** `trdl-write-configure-build-secrets`

Добавить секрет сборки
...
| Параметр | Тип | Обязательный | Описание |
|----------|-----|--------------|----------|
| `data` | string | да | Данные секрета |
| `id` | string | да | Идентификатор секрета |
...
**ID операции:** `trdl-delete-configure-build-secrets-id`

Удалить секрет сборки

#### Параметры

| Параметр | Тип | Расположение | Обязательный | Описание |
|----------|-----|--------------|--------------|----------|
| `id` | string | path | да | Идентификатор секрета |
| `trdl_mount_path` | string | path | да | Путь, по которому был смонтирован бэкэнд |
...
**ID операции:** `trdl-write-configure-delivery_kit_elf_signing`

Настроить подпись ELF-бинарников через Delivery Kit
...
| Параметр | Тип | Обязательный | Описание |
|----------|-----|--------------|----------|
| `certificate` | string | да | Данные сертификата в кодировке base64 |
| `intermediates` | string | нет | Цепочка сертификатов (промежуточные и корневой) в кодировке base64, единый PEM-пакет |
| `key` | string | да | Данные закрытого ключа в кодировке base64 или ссылка на ключ Vault в форме `hashivault://<key>`. При использовании ссылки `hashivault://` настройте параметры `vault_*` |
| `password` | string | нет | Пароль закрытого ключа. Не должен задаваться, когда `key` — ссылка `hashivault://` |
| `vault_addr` | string | нет | Адрес сервера Vault. Применяется только когда `key` — ссылка `hashivault://` |
| `vault_auth_path` | string (default: ar) | нет | Путь монтирования метода аутентификации Vault. Применяется только когда `key` — ссылка `hashivault://` |
| `vault_auth_role_id` | string | нет | RoleID AppRole для аутентификации в Vault. Применяется только когда `key` — ссылка `hashivault://` |
| `vault_auth_secret_id` | string | нет | SecretID AppRole для аутентификации в Vault. Применяется только когда `key` — ссылка `hashivault://` |
| `vault_transit_path` | string | нет | Путь монтирования движка transit Vault. Применяется только когда `key` — ссылка `hashivault://` |
...
**ID операции:** `trdl-delete-configure-delivery_kit_elf_signing`
...
**ID операции:** `trdl-write-configure-git_credential`
...
**ID операции:** `trdl-delete-configure-git_credential`
...
**ID операции:** `trdl-read-configure-last_published_git_commit`

Прочитать или удалить последний опубликованный Git-коммит
...
**ID операции:** `trdl-delete-configure-last_published_git_commit`
...
**ID операции:** `trdl-read-configure-pgp_signing_key`

Настроить PGP-ключ для подписи артефактов релиза
...
**ID операции:** `trdl-delete-configure-pgp_signing_key`

Удалить текущий PGP-ключ подписи
...
**ID операции:** `trdl-read-configure-trusted_pgp_public_key`

Настроить доверенные открытые ключи PGP
...
**ID операции:** `trdl-write-configure-trusted_pgp_public_key`
...
| Параметр | Тип | Обязательный | Описание |
|----------|-----|--------------|----------|
| `name` | string | да | Имя ключа |
| `public_key` | string | да | Данные ключа |
...
**ID операции:** `trdl-read-configure-trusted_pgp_public_key-name`

Прочитать или удалить настроенный доверенный открытый ключ PGP
...
**ID операции:** `trdl-delete-configure-trusted_pgp_public_key-name`
...
**ID операции:** `trdl-write-publish`

Опубликовать каналы релиза
...
| Параметр | Тип | Обязательный | Описание |
|----------|-----|--------------|----------|
| `git_password` | string | нет | Пароль Git |
| `git_username` | string | нет | Имя пользователя Git |
...
**ID операции:** `trdl-write-release`
...
**ID операции:** `trdl-read-task-configure`

Настроить менеджер задач
...
**ID операции:** `trdl-write-task-configure`
...
| Параметр | Тип | Обязательный | Описание |
|----------|-----|--------------|----------|
| `task_history_limit` | integer (default: 10) | нет | Лимит истории задач |
| `task_timeout` | integer (default: 30m) | нет | Таймаут задачи |
...
**ID операции:** `trdl-read-task-uuid`
...
| Параметр | Тип | Расположение | Обязательный | Описание |
|----------|-----|--------------|--------------|----------|
| `uuid` | string | path | да | UUID задачи |
| `trdl_mount_path` | string | path | да | Путь, по которому был смонтирован бэкэнд |
...
**ID операции:** `trdl-write-task-uuid-cancel`
...
**ID операции:** `trdl-read-task-uuid-log`
...
| Параметр | Тип | Расположение | Обязательный | Описание |
|----------|-----|--------------|--------------|----------|
| `limit` | integer | query | нет | Ограничение числа символов |
| `offset` | integer | query | нет | Смещение |
| `uuid` | string | path | да | UUID задачи |
| `trdl_mount_path` | string | path | да | Путь, по которому был смонтирован бэкэнд |
