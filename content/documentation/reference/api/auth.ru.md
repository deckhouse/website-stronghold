...
**ID операции:** `kerberos-read-configuration`

Настраивает keytab Kerberos и сервисный аккаунт.

#### Параметры

| Параметр | Тип | Расположение | Обязательный | Описание |
|----------|-----|--------------|--------------|----------|
| `kerberos_mount_path` | string | path | да | Путь, по которому был смонтирован бэкэнд |
...
**ID операции:** `kerberos-configure`
...
| Параметр | Тип | Обязательный | Описание |
|----------|-----|--------------|----------|
| `add_group_aliases` | boolean | нет | Если установлено значение true, возвращает все группы, найденные в LDAP, как псевдонимы групп. |
| `keytab` | string | нет | keytab в кодировке Base64. |
| `remove_instance_name` | boolean | нет | Удалить instance/FQDN из имён principal keytab. |
| `service_account` | string | нет | Сервисный аккаунт. |
...
**ID операции:** `kerberos-read-ldap-configuration`
...
**ID операции:** `kerberos-configure-ldap`
...
**ID операции:** `kerberos-list-groups`
...
**ID операции:** `kerberos-read-group`
...
**ID операции:** `kerberos-write-group`
...
**ID операции:** `kerberos-delete-group`
...
| Параметр | Тип | Обязательный | Описание |
|----------|-----|--------------|----------|
| `authorization` | string | нет | Заголовок авторизации SPNEGO. Обязателен. |
...
**ID операции:** `userpass-read-policies-password`

Установить, прочитать или удалить политику паролей, применяемую для userpass.

#### Параметры

| Параметр | Тип | Расположение | Обязательный | Описание |
|----------|-----|--------------|--------------|----------|
| `policy_name` | string | path | да | Имя политики паролей, применяемой для userpass. |
| `userpass_mount_path` | string | path | да | Путь, по которому был смонтирован бэкэнд |
...
| Параметр | Тип | Обязательный | Описание |
|----------|-----|--------------|----------|
| `policy_name` | string | нет | Имя политики паролей, применяемой для userpass. |

### POST /auth/{userpass_mount_path}/password-policy/{policy_name}

**ID операции:** `userpass-write-policies-password`
...
**ID операции:** `userpass-delete-policies-password`
...
**ID операции:** `userpass-read-policies-password-deprecated`
...
**ID операции:** `userpass-write-policies-password-deprecated`
...
**ID операции:** `userpass-delete-policies-password-deprecated`
...
**ID операции:** `webauthn-read-configuration`
...
| Параметр | Тип | Обязательный | Описание |
|----------|-----|--------------|----------|
| `auto_registration` | boolean | нет | Могут ли новые пользователи регистрироваться самостоятельно. |
| `rp_display_name` | string | нет | Отображаемое имя для Relying Party |
| `rp_id` | string | нет | Идентификатор Relying Party. |
| `rp_origins` | array | нет | Разрешённые источники для WebAuthn. |

### POST /auth/{webauthn_mount_path}/config

**ID операции:** `webauthn-write-configuration`
...
| Параметр | Тип | Обязательный | Описание |
|----------|-----|--------------|----------|
| `auto_registration` | boolean (default: False) | нет | Если true, новые пользователи могут регистрироваться самостоятельно. Если false (по умолчанию), регистрироваться могут только предварительно созданные пользователи. |
| `rp_display_name` | string | нет | Отображаемое имя для Relying Party |
| `rp_id` | string | нет |  |
...
**ID операции:** `webauthn-begin-login`
...
| Параметр | Тип | Обязательный | Описание |
|----------|-----|--------------|----------|
| `mediation` | string | нет | Необязательное требование mediation для учетных данных |
| `publicKey` | object | нет | PublicKeyCredentialRequestOptions для navigator.credentials.get() |

### POST /auth/{webauthn_mount_path}/login/finish

**ID операции:** `webauthn-finish-login`
...
**ID операции:** `webauthn-begin-registration`
...
| Параметр | Тип | Обязательный | Описание |
|----------|-----|--------------|----------|
| `registration_code` | string | нет | Одноразовый код регистрации, возвращённый при создании пользователя |
...
|----------|-----|--------------|----------|
| `mediation` | string | нет | Необязательное требование mediation для учетных данных |
| `publicKey` | object | нет | PublicKeyCredentialCreationOptions для navigator.credentials.create() |

### POST /auth/{webauthn_mount_path}/register/finish

**ID операции:** `webauthn-finish-registration`
...
| Параметр | Тип | Обязательный | Описание |
|----------|-----|--------------|----------|
| `message` | string | нет | Сообщение о статусе регистрации |
| `username` | string | нет | Зарегистрированное имя пользователя |

### GET /auth/{webauthn_mount_path}/user

**ID операции:** `webauthn-list-users`
...
**ID операции:** `webauthn-read-user`
...
| Параметр | Тип | Обязательный | Описание |
|----------|-----|--------------|----------|
| `credential_ids` | array | нет | Идентификаторы учетных данных в кодировке base64url |
| `credentials` | integer | нет | Количество зарегистрированных учетных данных WebAuthn |
...
| `token_ttl` | integer | нет | Начальный ttl генерируемого токена |
| `token_type` | string (default: default-service) | нет | Тип генерируемого токена, сервисный или пакетный |
| `user_id_b64` | string | нет | Идентификатор пользователя WebAuthn в кодировке base64url |
| `username` | string | нет | Имя пользователя |

### POST /auth/{webauthn_mount_path}/user/{name}

**ID операции:** `webauthn-write-user`
...
|----------|-----|--------------|----------|
| `display_name` | string | нет | Отображаемое имя пользователя |
| `registration_code` | string | нет | Одноразовый код регистрации (возвращается только при создании) |
| `username` | string | нет | Имя пользователя |

### DELETE /auth/{webauthn_mount_path}/user/{name}

**ID операции:** `webauthn-delete-user`
...
**ID операции:** `webauthn-delete-credential`
...
**ID операции:** `webauthn-generate-registration-code`

Сгенерировать новый одноразовый код регистрации для пользователя
...
| Параметр | Тип | Обязательный | Описание |
|----------|-----|--------------|----------|
| `registration_code` | string | нет | Одноразовый код регистрации |
| `username` | string | нет | Имя пользователя |

### POST /auth/{webauthn_mount_path}/user/{name}/policies

**ID операции:** `webauthn-update-policies`
