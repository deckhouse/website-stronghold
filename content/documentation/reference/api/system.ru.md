...
| Параметр | Тип | Обязательный | Описание |
|----------|-----|--------------|----------|
| `otp` | string | нет | Указывает одноразовый пароль для шифрования созданного токена. |
...
| Параметр | Тип | Обязательный | Описание |
|----------|-----|--------------|----------|
| `counts` | integer | нет | Количество совпадающих аренд на точку монтирования |
...
**ID операции:** `system-list-managed-keys-type`

Получить список настроенных управляемых ключей.

#### Параметры

| Параметр | Тип | Расположение | Обязательный | Описание |
|----------|-----|--------------|--------------|----------|
| `type` | string | path | да | Тип управляемого ключа. |
| `list` | string (true) | query | да | Должно быть установлено значение `true`. |
...
**ID операции:** `system-read-managed-keys-type-name`

Создание, обновление и чтение конфигураций управляемых ключей.
...
**ID операции:** `system-write-managed-keys-type-name`
...
| Параметр | Тип | Обязательный | Описание |
|----------|-----|--------------|----------|
| `allow_generate_key` | boolean | нет | Если true, позволяет пользователям ключа инициировать создание ключа. Если false и требуется создание, операция завершится неудачей. |
| `allow_replace_key` | boolean | нет | Если true, позволяет пользователям ключа предоставлять материал ключа, который может заменить ранее присутствующие ключи. Значение allow_store_key=false переопределяет это поведение. |
...
| `curve` | string | нет | Для механизмов ECDSA желаемая эллиптическая кривая для создания ключа: P256, P384 или P521. |
| `endpoint` | string | нет | [yandexcloudkms] Пользовательская конечная точка API Yandex Cloud (необязательно). |
| `force_rw_session` | boolean (default: False) | нет | Если true, принудительно использует сеансы чтения/записи на HSM для работы с некорректно работающими HSM. |
| `key_bits` | integer | нет | Для механизмов RSA желаемая длина ключа в битах для создания: 2048, 3072 или 4096. |
| `key_id` | string | нет | ID ключа PKCS#11 для использования. Поскольку ID ключей создаются HSM, это ошибка, если ключ ещё не существует. Должно быть указано это значение или key_label. |
| `key_label` | string | нет | Метка ключа PKCS#11 для использования. Если ключ не существует и создание включено, это метка, которая будет присвоена созданному ключу. Должно быть указано это значение или key_id. |
| `kms_key_id` | string | нет | [yandexcloudkms] ID ключа в Yandex Cloud KMS (симметричный или асимметричный). |
| `library` | string | нет | Имя библиотеки доступа к управляемому ключу, как определено в конфигурации сервера. |
| `max_parallel` | integer | нет | Максимальное число одновременных операций, которые могут быть отправлены на HSM одновременно. |
| `mechanism` | string | нет | Механизм для данного ключа, указанный как десятичная или шестнадцатеричная (с префиксом 0x) строка. |
...
| `pin` | string | нет | PIN доступа для слота. |
| `service_account_key_json` | string | нет | [yandexcloudkms] JSON авторизованного ключа сервисного аккаунта Yandex Cloud. Хранится внутри Stronghold, не в файловой системе. Взаимоисключающий с oauth_token. |
| `slot` | string | нет | Номер слота для использования, указанный как строка (например, "0"). |
| `token_label` | string | нет | Метка токена слота для использования. |
...
**ID операции:** `system-delete-managed-keys-type-name`
...
**ID операции:** `system-write-managed-keys-type-name-test-sign`

Протестировать управляемый ключ путём подписания и проверки некоторых случайных данных.
...
| Параметр | Тип | Обязательный | Описание |
|----------|-----|--------------|----------|
| `hash_algorithm` | string (default: sha2-256) | нет | Хэш для контрольной суммы тестирования. Если опустить, вычисляется на основе механизма PKCS#11 управляемого ключа для GOST (streebog-256 / streebog-512); в противном случае sha2-256. |
| `use_pss` | boolean (default: False) | нет | Для управляемых ключей на основе RSA попытаться выполнить подпись с использованием PSS |
...
**ID операции:** `enterprise-stub-list-mfa-method`
...
**ID операции:** `enterprise-stub-read-mfa-method-duo-name`
...
**ID операции:** `enterprise-stub-write-mfa-method-duo-name`
...
**ID операции:** `enterprise-stub-delete-mfa-method-duo-name`
...
**ID операции:** `enterprise-stub-read-mfa-method-multifactor-name`
...
**ID операции:** `enterprise-stub-write-mfa-method-multifactor-name`
...
**ID операции:** `enterprise-stub-delete-mfa-method-multifactor-name`
...
**ID операции:** `enterprise-stub-read-mfa-method-okta-name`
...
**ID операции:** `enterprise-stub-write-mfa-method-okta-name`
...
**ID операции:** `enterprise-stub-delete-mfa-method-okta-name`
...
**ID операции:** `enterprise-stub-read-mfa-method-pingid-name`
...
**ID операции:** `enterprise-stub-write-mfa-method-pingid-name`
...
**ID операции:** `enterprise-stub-delete-mfa-method-pingid-name`
...
**ID операции:** `enterprise-stub-read-mfa-method-totp-name`
...
**ID операции:** `enterprise-stub-write-mfa-method-totp-name`
...
**ID операции:** `enterprise-stub-delete-mfa-method-totp-name`
...
**ID операции:** `enterprise-stub-write-mfa-method-totp-name-admin-destroy`
...
**ID операции:** `enterprise-stub-write-mfa-method-totp-name-admin-generate`
...
**ID операции:** `enterprise-stub-read-mfa-method-totp-name-generate`
...
| `passthrough_request_headers` | array | нет |  |
| `plugin_version` | string | нет | Семантическая версия плагина для использования. |
| `replication_config` | object | нет | Конфигурация репликации KV. |
| `token_type` | string | нет | Тип выпускаемого токена (сервисный или пакетный). |
| `user_lockout_counter_reset_duration` | integer | нет |  |
| `user_lockout_disable` | boolean | нет |  |
| `user_lockout_duration` | integer | нет |  |
...
| `audit_non_hmac_request_keys` | array | нет | Список ключей в объекте данных запроса, которые не будут обрабатываться HMAC устройствами аудита. |
| `audit_non_hmac_response_keys` | array | нет | Список ключей в объекте данных ответа, которые не будут обрабатываться HMAC устройствами аудита. |
| `cmd_enable_repl` | boolean | нет | Включить или отключить репликацию для этой точки монтирования. |
| `default_lease_ttl` | string | нет | TTL аренды по умолчанию для этого монтирования. |
| `description` | string | нет | Удобное описание для данного бэкенда учетных данных. |
| `listing_visibility` | string | нет | Определяет видимость монтирования в конечной точке листинга, специфичной для пользовательского интерфейса. Принимаются значения 'unauth' и 'hidden', при этом пустое значение по умолчанию ('') ведет себя как 'hidden'. |
| `max_lease_ttl` | string | нет | Максимальное значение TTL аренды для данного монтирования. |
...
| `passthrough_request_headers` | array | нет | Список заголовков, которые необходимо включить в белый список и передать из запроса в плагин. |
| `plugin_version` | string | нет | Семантическая версия плагина для использования. |
| `src_address` | string | нет | Адрес исходного кластера Stronghold для репликации. |
| `src_ca_cert` | string | нет | Сертификат CA для TLS-соединения с исходным кластером. |
| `src_mount_path` | string | нет | Путь монтирования движка KV на исходном кластере. |
| `src_secret_path` | array | нет | Список путей секретов для репликации из исходного кластера. |
| `src_token` | string | нет | Токен для доступа к API удаленного сервера. Взаимоисключает src_wrapping_token. |
| `src_wrapping_token` | string | нет | Wrapping token, который будет развёрнут на удаленном сервере для получения токена репликации. Взаимоисключает src_token. |
| `sync_period_min` | integer | нет | Период синхронизации репликации в минутах. |
| `token_type` | string | нет | Тип выпускаемого токена (сервисный или пакетный). |
| `user_lockout_config` | object | нет | Конфигурация блокировки пользователя для передачи в бэкэнд. Это должен быть json-объект со строковыми ключами и значениями. |
...
**ID операции:** `rekey-recovery-key-attempt-read-progress`

Читает конфигурацию и прогресс текущей попытки переопределения ключа восстановления.
...
**ID операции:** `rekey-recovery-key-attempt-initialize`

Инициирует новую попытку переопределения ключа восстановления.

#### Параметры тела запроса

| Параметр | Тип | Обязательный | Описание |
|----------|-----|--------------|----------|
| `backup` | boolean | нет | Указывает, должен ли Vault сохранять резервную копию в открытом виде ключей, зашифрованных PGP, если они используются. |
| `pgp_keys` | array | нет | Указывает массив открытых ключей PGP, используемых для шифрования выходных ключей восстановления. |
| `require_verification` | boolean | нет | Включение функции проверки |
| `secret_shares` | integer | нет | Укажите количество долей, на которые нужно разделить ключ восстановления. |
| `secret_threshold` | integer | нет | Указывает число долей, необходимых для восстановления ключа восстановления. |
| `stored_shares` | integer | нет | Указывает количество долей, которые должны быть зашифрованы HSM и сохранены для автоматического раскрытия. В настоящее время должно быть таким же, как `secret_shares`. |
...
**ID операции:** `rekey-recovery-key-attempt-cancel`

Отменить любое текущее переопределение ключа восстановления.
...
**ID операции:** `rekey-recovery-key-attempt-update`

Введите единственную долю ключа восстановления для продолжения переопределения ключа Vault.

#### Параметры тела запроса

| Параметр | Тип | Обязательный | Описание |
|----------|-----|--------------|----------|
| `key` | string | нет | Указывает единственную долю ключа восстановления. |
...
**ID операции:** `rekey-recovery-key-verification-read-progress`

Прочитать конфигурацию и прогресс текущей попытки проверки переопределения ключа восстановления.
...
**ID операции:** `rekey-recovery-key-verification-update`

Введите единственный новый доля ключа восстановления для продолжения операции проверки переопределения.

#### Параметры тела запроса

| Параметр | Тип | Обязательный | Описание |
|----------|-----|--------------|----------|
| `key` | string | нет | Указывает единственную долю ключа восстановления из нового набора долей. |
...
**ID операции:** `rekey-recovery-key-verification-cancel`

Отменить любую текущую операцию проверки переопределения ключа восстановления.
...
| `nonce` | string | нет | Указывает nonce операции переопределения. Если переопределение было инициировано в последние 10 минут, вы должны предоставить nonce для отмены операции. |
...
| `secret_shares` | integer | нет | Указывает количество долей, на которые нужно разделить ключ разблокировки. |
| `secret_threshold` | integer | нет | Указывает количество долей, необходимых для восстановления ключа unseal. Это значение должно быть меньше или равно secret_shares. При использовании Stronghold HSM с автоматической разблокировкой это значение должно быть равно secret_shares. |
| `stored_shares` | integer | нет | Указывает число долей, которые должны быть зашифрованы HSM и сохранены для автоматического открытия. В настоящее время должно быть равно secret_shares. |
...
**ID операции:** `system-read-replication-dr-secondary-generate-operation-token-attempt`
...
**ID операции:** `system-write-replication-dr-secondary-generate-operation-token-attempt`
...
**ID операции:** `system-delete-replication-dr-secondary-generate-operation-token-attempt`
...
**ID операции:** `system-write-replication-dr-secondary-generate-operation-token-update`
...
| `aws_s3_disable_tls` | boolean (default: False) | нет | Отключить TLS для конечной точки S3. Используйте только для тестирования, обычно вместе с `aws_s3_endpoint`. |
...
| `interval` | integer | да | Время между снимками. Должно быть не менее 3m. Принимает целое число секунд или строку длительности. Строки длительности могут сочетать единицы длительности Go ns, us/µs, ms, s, m и h, например 15m, 1h, 1h30m или 24h3m. Отдельное значение дня, такое как 1d, также принимается; сочетайте дни с меньшими единицами, используя эквивалентные часы, например 24h3m. |
...
| `retry_enabled` | boolean (default: False) | нет | Включить повторные попытки при неудачных загрузках на S3. Применяется только когда `storage_type=aws-s3`. |
| `retry_max_attempts` | integer (default: 5) | нет | Максимальное число повторных попыток при неудачных загрузках на S3. График задержки: 1m, 5m, 15m, затем 30m. Применяется только когда `storage_type=aws-s3`. |
| `storage_type` | string (local, aws-s3) | да | Один из значений "local" или "aws-s3". Остальные параметры, описанные ниже, специфичны для выбранного `storage_type` и имеют соответствующий префикс. |
