...
| `username_format` | string | нет | Строка шаблона для преобразования имён Identity в имена методов MFA. Значения для подстановки следует помещать в {{}}. Например, "{{identity.entity.aliases.auth_userpass_abc123.name}}@example.com". Если оставить пусто, будет использоваться поле name объекта Entity как есть. |
...
**ID операции:** `mfa-list-multifactor-methods`
...
**ID операции:** `mfa-create-multifactor-method`
...
| Параметр | Тип | Обязательный | Описание |
|----------|-----|--------------|----------|
| `api_url` | string | нет | Базовый URL API Multifactor. По умолчанию установлено значение "<https://api.multifactor.ru>". |
| `method_name` | string | нет | Уникальный идентификатор имени для данного метода MFA. |
| `nas_identifier` | string | нет | Идентификатор Multifactor NAS из параметров ресурса. |
| `shared_secret` | string | нет | Общий секрет Multifactor из параметров ресурса. |
| `timeout_seconds` | integer | нет | Максимальное число секунд ожидания подтверждения push от Multifactor. По умолчанию 90. Минимум 65. |
| `username_format` | string | нет | Строка шаблона для преобразования имён Identity в Multifactor identities. Значения для подстановки следует помещать в {{}}. Например, "{{identity.entity.aliases.auth_userpass_abc123.name}}@example.com". Если оставить пусто, будет использоваться поле name объекта Entity как есть. |
...
**ID операции:** `mfa-read-multifactor-method-configuration`
...
**ID операции:** `mfa-configure-multifactor-method`
...
**ID операции:** `mfa-delete-multifactor-method`
...
| `username_format` | string | нет | Строка шаблона для преобразования имён Identity в имена методов MFA. Значения для подстановки следует помещать в {{}}. Например, "{{identity.entity.name}}@example.com". Если оставить пусто, будет использоваться поле name объекта Entity как есть. |
