...
Эта страница нужна для системных операций Deckhouse Stronghold через API. Здесь обычно описываются endpoint'ы для управления системными настройками, монтированием механизмов секретов, политиками, токенами, конфигурацией и служебными операциями. Раздел полезен администраторам и инженерам автоматизации, которые управляют Stronghold программно.
...
**ID операции:** `managed-keys-list-keys`

Список управляемых ключей указанного типа
...
**ID операции:** `managed-keys-read-key`

Прочитать конфигурацию управляемого ключа
...
**ID операции:** `managed-keys-update`

Обновить управляемый ключ
...
| Параметр | Тип | Обязательный | Описание |
| --- | --- | --- | --- |
| `allow_generate_key` | boolean | нет | Если true, разрешает пользователям ключа запускать генерацию ключа. |
...
**ID операции:** `managed-keys-delete-key`

Удалить управляемый ключ
...
**ID операции:** `managed-keys-test-sign`

Проверить подпись управляемым ключом
...
| Параметр | Тип | Обязательный | Описание |
| --- | --- | --- | --- |
| `hash_algorithm` | string (default: ) | нет | Алгоритм хеширования, используемый при подписи/проверке случайных данных. |
