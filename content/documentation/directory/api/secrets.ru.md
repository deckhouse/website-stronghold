...
Эта страница нужна для работы с механизмами секретов Deckhouse Stronghold через API. Здесь собраны endpoint'ы для чтения, записи, генерации и управления секретами в разных secrets engine, включая KV, PKI, Transit, SSH, TOTP, LDAP, Kubernetes и другие. Раздел поможет быстро найти нужный путь API для конкретного механизма секретов.
