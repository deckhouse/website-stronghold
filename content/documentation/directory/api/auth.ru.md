...
Эта страница нужна для работы с методами аутентификации Deckhouse Stronghold. Здесь собраны endpoint'ы, которые используются для входа в Stronghold, получения токенов и настройки auth method. Раздел будет полезен администраторам, разработчикам и сервисам, которым нужно автоматизировать аутентификацию через API.
